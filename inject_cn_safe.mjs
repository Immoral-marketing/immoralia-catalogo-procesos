import fs from 'fs';
// We don't import because it's a .ts file and we are running in node
// Instead we'll use a hack or just copy the data here or read it from the file system as text

const processesFilePath = 'src/data/processes.ts';
const newProcessesFilePath = 'new_processes.ts';

let content = fs.readFileSync(processesFilePath, 'utf8');

// Simple parser for the new_processes.ts (specific to our structure)
const newProcessesRaw = fs.readFileSync(newProcessesFilePath, 'utf8');
// This is risky, let's just use the data I have from the view_file earlier.
// Actually, I'll just write the final script with the data embedded to be 100% safe.

const constructorasProcesses = [
  {
    id: "CN1",
    codigo: "CN1",
  ...
  },
  {
    ...
    personalizacion: "Pídenos diseñar el flujo de tu embudo específico para que el gráfico encaje al 100% con tu reality comercial.",
...
const categoryMapping = {
  "Ventas y CRM": "G",
  "Análisis y Reporte": "H",
  "Atención y Clientes": "I",
  "Operaciones y Firma": "J",
  "Operaciones y Contratos": "J",
  "Internos Agencias": "D"
};

const newCategories = [
  { id: "G", name: "Ventas y CRM", emoji: "📈" },
  { id: "H", name: "Análisis y Reporte", emoji: "📊" },
  { id: "I", name: "Atención y Clientes", emoji: "🤝" },
  { id: "J", name: "Operaciones y Firma", emoji: "✍️" }
];

const updatedProcesses = constructorasProcesses.map(p => {
  const catId = categoryMapping[p.categoria] || p.categoria;
  return {
    ...p,
    categoria: catId,
    sectores: [...new Set([...p.sectores, "Construcción & Reformas"])]
  };
});

// 3. Inject Categories
let categoriesBlockMatch = content.match(/export const categories = \[([\s\S]*?)\];/);
if (categoriesBlockMatch) {
  let categoriesText = categoriesBlockMatch[1];
  newCategories.forEach(cat => {
    if (!categoriesText.includes(`id: "${cat.id}"`)) {
      categoriesText += `  { id: "${cat.id}", name: "${cat.name}", emoji: "${cat.emoji}" },\n`;
    }
  });
  content = content.replace(categoriesBlockMatch[1], categoriesText);
}

// 4. Inject Processes
const processesEndIndex = content.lastIndexOf('];\n\nexport const categories =');
if (processesEndIndex !== -1) {
  const processesString = updatedProcesses.map(p => {
    return `  {\n` +
      `    id: "${p.id}",\n` +
      `    codigo: "${p.codigo}",\n` +
      `    slug: "${p.slug}",\n` +
      ...
      `    summary: ${JSON.stringify(p.summary, null, 2).replace(/\n/g, '\n    ')},\n` +
      `    indicators: ${JSON.stringify(p.indicators, null, 2).replace(/\n/g, '\n    ')},\n` +
      `    how_it_works_steps: ${JSON.stringify(p.how_it_works_steps, null, 2).replace(/\n/g, '\n    ')},\n` +
      `    customization: ${JSON.stringify(p.customization, null, 2).replace(/\n/g, '\n    ')},\n` +
      `    faqs: ${JSON.stringify(p.faqs, null, 2).replace(/\n/g, '\n    ')},\n` +
      `    pasos: ${JSON.stringify(p.pasos, null, 2).replace(/\n/g, '\n    ')},\n` +
      `    personalizacion: "${p.personalizacion}",\n` +
      `    related_processes: ${JSON.stringify(p.related_processes)},\n` +
      `    sectores: ${JSON.stringify(p.sectores)},\n` +
      `    herramientas: ${JSON.stringify(p.herramientas)},\n` +
      `    integration_domains: ${JSON.stringify(p.integration_domains)},\n` +
      `    landing_slug: "${p.landing_slug}"\n` +
      `  }`;
  }).join(',\n');

  content = content.slice(0, processesEndIndex) + ',\n' + processesString + '\n' + content.slice(processesEndIndex);
}

fs.writeFileSync(processesFilePath, content);
console.log('Successfully injected categories and processes.');
