...
let newBlocks = blocks.map((block, i) => {
  ...
  if (isMissingCust) {
    ...
    if (newBlock.includes('    faqs: [\n')) newBlock = newBlock.replace('    faqs: [\n', cust + '    faqs: [\n');
    else if (newBlock.includes('    faqs:')) newBlock = newBlock.replace('    faqs:', cust + '    faqs:');
    else if (newBlock.includes('    pasos:')) newBlock = newBlock.replace('    pasos:', cust + '    pasos:');
    else newBlock = newBlock.replace(/\n  \}(\n|$)/, '\n' + cust + '  }\n');
  }
  
  if (isMissingDemo) {
    const demo = `    demo: { video_url: "PENDING" },\n`;
    if (newBlock.includes('    faqs: [\n')) newBlock = newBlock.replace('    faqs: [\n', demo + '    faqs: [\n');
    else if (newBlock.includes('    faqs:')) newBlock = newBlock.replace('    faqs:', demo + '    faqs:');
...
if (fs.existsSync(jsonPath)) {
  ...
  jsonString = jsonString.replace(/## Personalización\nnull\n/g, '## Personalización\nOpciones estándar de plataformas activadas en UI.\n\n');
