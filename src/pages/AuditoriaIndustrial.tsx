...
import {
  ...
  type IndAuditLevel,
  type IndAuditQuestion,
} from "@/data/auditoriaIndustrialData";
import { processes } from "@/data/processes";
import { downloadIndAuditPdf, type IndAuditState } from "@/lib/auditoriaIndustrialPdf";
...
const AuditoriaIndustrial = () => {
  ...
  const total = IND_AUDIT_QUESTIONS.length;
  const stepNum = cur + 1;
  const pct = Math.round((stepNum / (total + 1)) * 100);

  const q = IND_AUDIT_QUESTIONS[cur];
  ...
  const pick = (idx: number, val: string | number) => {
    const question = IND_AUDIT_QUESTIONS[idx];
  ...
  const finish = () => {
    ...
    const byBlock: Record<IndAuditBlockId, number[]> = {
      B1: [],
      B2: [],
      B3: [],
      B4: [],
    ...
    IND_AUDIT_QUESTIONS.forEach((qq, i) => {
      if (qq.type === "scale" && answers[i] !== undefined) {
        byBlock[qq.block].push(answers[i] as number);
      }
    });
    const blockScores = {} as Record<IndAuditBlockId, number>;
    IND_AUDIT_BLOCK_KEYS.forEach((b) => {
      const arr = byBlock[b];
      if (!arr.length) {
        blockScores[b] = 0;
        return;
    ...
    const global = Math.round(
      IND_AUDIT_BLOCK_KEYS.reduce((a, b) => a + blockScores[b], 0) / IND_AUDIT_BLOCK_KEYS.length,
    );
    const level =
      IND_AUDIT_LEVELS.find((l) => global >= l.min && global <= l.max) ?? IND_AUDIT_LEVELS[0];
    ...
    if (!GHL_WEBHOOK_URL.includes("LOCATION_ID")) {
      const payload = {
        ...
        custom_fields: {
          ...
          audit_tipo_empresa: IND_AUDIT_TIPO_EMPRESA[(answers[0] as string) || "A"] || "—",
          audit_plantilla: IND_AUDIT_TAMANO_PLANTILLA[(answers[1] as string) || "A"] || "—",
          audit_canales: (Array.isArray(answers[2])
            ? (answers[2] as string[])
            : [answers[2] as string].filter(Boolean)
          )
            .map((k) => IND_AUDIT_CANALES[k] || k)
            .join(", "),
          audit_madurez_digital:
            IND_AUDIT_SISTEMAS_DIGITALES[(answers[15] as string) || "A"] || "—",
          audit_concentracion_clientes: (answers[16] as string) || "—",
          audit_certificaciones: (Array.isArray(answers[17])
            ? (answers[17] as string[])
            : [answers[17] as string].filter(Boolean)
          )
            .map((k) => IND_AUDIT_CERTIFICACIONES[k] || k)
            .join(", ") || "—",
          audit_resiliencia: IND_AUDIT_RIESGO_CLAVE[(answers[18] as string) || "A"] || "—",
          audit_priority: (Array.isArray(answers[19])
            ? (answers[19] as string[])
            : [answers[19] as string].filter(Boolean)
          )
            .map((k) => IND_AUDIT_PRIORIDADES[k] || k)
            .join(", ") || "—",
          audit_sector: "industrial",
  ...
  const handleDownload = () => {
    ...
    downloadIndAuditPdf(state, setPdfBusy);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-white selection:bg-yellow-500/30 font-sans">
      ...
      {screen === "intro" && <IntroScreen onStart={start} total={total} />}
...
/* ───────────────────────── INTRO ───────────────────────── */
const IntroScreen = ({ onStart, total }: { onStart: () => void; total: number }) => (
  <section className="relative py-24 md:py-32 overflow-hidden">
    <div className="absolute inset-0 bg-gradient-to-br from-yellow-500/8 via-transparent to-amber-600/8 pointer-events-none" />
    ...
    <div className="absolute top-1/2 right-0 -translate-y-1/2 w-96 h-96 bg-amber-600/8 blur-[160px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-3xl mx-auto text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-yellow-500/10 border border-yellow-500/20 mb-7">
          ...
          <span className="text-yellow-300 font-medium tracking-widest uppercase text-[11px]">
            Auditoría confidencial · Industrial / Producción 2026
          </span>
        </div>
        ...
        <h1 className="text-4xl md:text-6xl font-bold mb-7 tracking-tight leading-[1.05]">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-200">
            operativa de tu planta industrial
          </span>
          .
        </h1>

        <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-5 leading-relaxed">
          Te hacemos las preguntas que importan sobre cómo gestionas presupuestos, pedidos,
          calidad, compras, facturación y planta. Recibes un{" "}
        ...
        <div className="flex flex-wrap gap-2.5 justify-center mb-10">
          {[
            ...
            { icon: ListChecks, label: `${total} preguntas guiadas` },
        ...
        {[
          {
            ...
            desc: "Una nota de 0 a 100 con el nivel en el que está hoy tu empresa frente a una industrial moderna bien sistematizada.",
          },
          {
            n: "02",
            title: "Diagnóstico por área",
            desc: "Score detallado en las 6 áreas clave: clientes y presupuestos, pedidos y producción, calidad, compras, administración y equipo.",
          },
          {
            n: "03",
  ...
  <section className="py-12 md:py-20">
    <div className="container mx-auto px-6">
      <div className="max-w-2xl mx-auto">
        ...
        {q.help && (() => {
          // Para preguntas de escala, partimos el ayuda "1 = X; 5 = Y" en dos líneas
          // con el número resaltado en amarillo (color del sector).
          const m = q.type === "scale"
            ? q.help.match(/^\s*1\s*=\s*(.+?)\s*;\s*5\s*=\s*(.+?)\s*$/i)
            : null;
          if (m) {
            const [, low, high] = m;
            return (
              <div className="grid gap-2.5 mb-8 rounded-xl border border-white/5 bg-white/[0.02] p-4">
                <div className="flex gap-3 items-start">
                  <span className="shrink-0 inline-flex items-center justify-center w-7 h-7 rounded-full bg-yellow-500/15 border border-yellow-500/40 text-yellow-300 text-xs font-bold">
                    1
                  </span>
                  <p className="text-sm text-gray-300 leading-relaxed pt-0.5">{low}</p>
                </div>
                <div className="flex gap-3 items-start">
                  <span className="shrink-0 inline-flex items-center justify-center w-7 h-7 rounded-full bg-yellow-500/15 border border-yellow-500/40 text-yellow-300 text-xs font-bold">
                    5
                  </span>
                  <p className="text-sm text-gray-300 leading-relaxed pt-0.5">{high}</p>
                </div>
              </div>
            );
          }
          return <p className="text-gray-400 mb-8 leading-relaxed">{q.help}</p>;
        })()}
              ...
              return (
                <button
                  ...
                  className={`group text-left p-4 rounded-xl border transition-all flex items-center gap-4 ${
                    selected
                      ? "bg-yellow-500/10 border-yellow-500/50 shadow-[0_0_24px_rgba(234,179,8,0.18)]"
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div>
              ...
              <Input
                ...
                placeholder="Ej. Metalúrgica del Norte S.L."
                className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 h-12"
              />
            </div>
            <div>
...
const ResultScreen = ({
  ...
  contact: Contact;
  result: { blockScores: Record<IndAuditBlockId, number>; global: number; level: IndAuditLevel };
  pdfBusy: boolean;
  onDownload: () => void;
}) => {
  const sorted = IND_AUDIT_BLOCK_KEYS.map((b) => ({ b, sc: result.blockScores[b] })).sort(
    (a, b) => b.sc - a.sc,
  );
  ...
  let ctaTitle = "Convirtamos tu planta en sistema.";
  let ctaSub =
    "Una conversación de 30 minutos para ver dónde está la fricción real entre comercial, producción y administración, qué ya tenéis montado y qué módulo puede generar impacto antes del próximo trimestre.";
  if (result.global < 50) {
    ctaTitle = "Cada hora de planta parada es margen perdido.";
    ctaSub =
      "Tu nivel indica que hay módulos que pueden activarse en menos de dos semanas con impacto visible desde el primer mes. Agenda 30 min con nuestro equipo y preparamos un plan modular para tu empresa.";
  } else if (result.global < 75) {
    ctaTitle = "Ya tienes una base. Falta hacerla sistema.";
    ctaSub =
      "Tienes piezas funcionando. Lo que falta es integrarlas y añadir la capa de trazabilidad, mantenimiento preventivo y cobros que convierte tu planta en una operación predecible sin depender de personas concretas.";
  } else {
    ctaTitle = "Estás listo para la siguiente capa de optimización.";
    ctaSub =
      "Tu operativa ya está sólida. Hablemos sobre análisis de rendimiento por máquina y turno, predicción de demanda y escalado del sistema a nuevas líneas o plantas sin perder control.";
  }
    ...
    <section className="py-12 md:py-16">
      <div className="container mx-auto px-6">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Score hero */}
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#1a1408] to-[#0d0a04] p-8 md:p-12 mb-6 grid grid-cols-1 md:grid-cols-[240px_1fr] gap-10 items-center">
          ...
          <div className="rounded-3xl border border-white/8 bg-white/[0.02] p-7 md:p-8 mb-6">
            ...
            <div className="grid gap-3.5">
              {IND_AUDIT_BLOCK_KEYS.map((b) => {
                const sc = result.blockScores[b];
                const color =
                  sc < 40 ? "bg-red-500" : sc < 70 ? "bg-amber-700" : "bg-yellow-400";
                const textColor =
                  sc < 40 ? "text-red-400" : sc < 70 ? "text-amber-500" : "text-yellow-400";
                return (
                  <div
                    key={b}
                    className="grid grid-cols-[48px_1fr_72px] gap-4 items-center p-4 rounded-xl bg-white/[0.02] border border-white/5"
                  >
                    <div className="w-12 h-12 rounded-full bg-[#1a1408] border border-yellow-500/30 flex items-center justify-center">
                      <span className="text-yellow-400 font-bold text-sm">{b}</span>
                    </div>
                    <div>
                      <div className="text-white font-semibold text-sm">
                        {IND_AUDIT_BLOCKS[b].name}
                      </div>
                      <div className="text-xs text-gray-500 mt-0.5 mb-2">
                        {IND_AUDIT_BLOCKS[b].short}
                      </div>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <div className="p-5 rounded-2xl border border-red-500/25 bg-red-500/[0.04]">
                <h4 className="text-red-400 text-xs uppercase tracking-widest font-bold mb-3 flex items-center gap-1.5">
                  ...
                  Dónde se pierden pedidos, margen y tiempo
                </h4>
                <ul className="space-y-2.5">
                  {weak.length ? (
                    weak.map((x) => (
                    ...
                    <li className="text-sm text-gray-400 leading-relaxed">
                      ...
                      optimización fina y escalado a nuevas líneas o plantas.
                    </li>
                  )}
                </ul>
              </div>
          ...
          <div className="rounded-3xl border border-white/8 bg-white/[0.02] p-7 md:p-8 mb-6">
            <div className="text-yellow-400 text-[11px] tracking-widest font-bold uppercase mb-1">
              Módulos prioritarios para tu empresa
            </div>
              ...
              {modules.map((m, i) => (
                ...
                >
                  <div className="text-[10px] tracking-widest text-yellow-400 font-bold uppercase mb-1">
                    {m.ref} · {IND_AUDIT_BLOCKS[m.block].name}
                  </div>
          ...
          {/* CTA final */}
          <div className="rounded-3xl bg-gradient-to-br from-[#1a1408] to-[#0d0a04] border border-yellow-500/20 p-8 md:p-10 text-center text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="inline-flex items-center justify-center gap-2 px-7 h-12 rounded-full bg-yellow-500 text-black font-bold hover:bg-yellow-400 transition-all"
              >
                Agendar llamada de 30 min <ArrowRight className="w-4 h-4" />
              </a>
              <Link
                to="/sector/industrial"
...
export default AuditoriaIndustrial;
