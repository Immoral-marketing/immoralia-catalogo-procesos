...
interface AdminPageProps {
  /**
   * SPEC-09: Datos pre-cargados server-side con @supabase/ssr.
   * - initialUser undefined  → sin datos SSR; cliente determina el estado.
   * - initialUser null       → servidor confirma sin sesión activa.
   * - initialUser User       → servidor confirma sesión activa.
   * - initialIsAdmin         → resultado de la query super_admins (solo si initialUser != null).
   */
  initialUser?: User | null
  initialIsAdmin?: boolean
}

export default function AdminPage({ initialUser, initialIsAdmin = false }: AdminPageProps = {}) {
  // Derivar el estado inicial desde los datos del servidor
  const [authState, setAuthState] = useState<AuthState>(() => {
    if (initialUser === undefined) return 'loading'      // sin datos SSR
    if (initialUser === null) return 'unauthenticated'  // sin sesión
    if (!initialIsAdmin) return 'not_admin'             // sesión pero sin permisos
    return 'authenticated'                              // sesión + admin
  });
  ...
  useEffect(() => {
    ...
    if (isRecoveryFlow) {
      ...
      // Estado ya determinado por el servidor — solo obtenemos el objeto session
      // para que AdminDashboard pueda acceder a los datos del usuario.
  ...
  // Breve instancia tras hidratación SSR: authState ya es 'authenticated'
  // pero session aún no llegó del cliente (getSession() es async).
  // Mostramos el loader hasta que session esté disponible.
  if (!session) {
    return <FullPageLoader />;
  }
