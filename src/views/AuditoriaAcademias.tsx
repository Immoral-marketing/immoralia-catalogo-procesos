...
import {
  ...
  type AcAuditBlockId,
  type AcAuditLevel,
  type AcAuditQuestion,
} from "@/data/auditoriaAcademiasData";
import { processes } from "@/data/processes";
import { downloadAcAuditPdf, type AcAuditState } from "@/lib/auditoriaAcademiasPdf";
...
const AuditoriaAcademias = () => {
  ...
  const total = AC_AUDIT_QUESTIONS.length;
  const stepNum = cur + 1;
  const pct = Math.round((stepNum / (total + 1)) * 100);

  const q = AC_AUDIT_QUESTIONS[cur];
  ...
  const pick = (idx: number, val: string | number) => {
    const question = AC_AUDIT_QUESTIONS[idx];
  ...
  const finish = () => {
    ...
    const byBlock: Record<AcAuditBlockId, number[]> = {
      B1: [],
      B2: [],
      B3: [],
      B4: [],
    ...
    AC_AUDIT_QUESTIONS.forEach((qq, i) => {
      if (qq.type === "scale" && answers[i] !== undefined) {
        byBlock[qq.block].push(answers[i] as number);
      }
    });
    const blockScores = {} as Record<AcAuditBlockId, number>;
    AC_AUDIT_BLOCK_KEYS.forEach((b) => {
      const arr = byBlock[b];
      if (!arr.length) {
        blockScores[b] = 0;
        return;
    ...
    const global = Math.round(
      AC_AUDIT_BLOCK_KEYS.reduce((a, b) => a + blockScores[b], 0) / AC_AUDIT_BLOCK_KEYS.length,
    );
    const level =
      AC_AUDIT_LEVELS.find((l) => global >= l.min && global <= l.max) ?? AC_AUDIT_LEVELS[0];
    ...
    if (!GHL_WEBHOOK_URL.includes("LOCATION_ID")) {
      const payload = {
        ...
        custom_fields: {
          ...
          audit_tipo_academia: AC_AUDIT_TIPO_ACADEMIA[(answers[0] as string) || "A"] || "—",
          audit_num_alumnos: AC_AUDIT_NUM_ALUMNOS[(answers[1] as string) || "A"] || "—",
          audit_canales: (Array.isArray(answers[2])
            ? (answers[2] as string[])
            : [answers[2] as string].filter(Boolean)
          )
            .map((k) => AC_AUDIT_CANALES[k] || k)
            .join(", "),
  ...
  const handleDownload = () => {
    ...
    downloadAcAuditPdf(state, setPdfBusy);
  };
      ...
      <nav className="border-b border-white/5 bg-black/60 backdrop-blur-md sticky top-0 z-50">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <button
            ...
            className="text-sm text-gray-400 hover:text-violet-300 transition-colors flex items-center gap-1.5"
          >
        ...
        <div className="sticky top-[65px] z-40 bg-[#0d0d0d]/95 backdrop-blur-md border-b border-white/5">
          <div className="container mx-auto px-6 py-3">
            ...
            <div className="flex justify-between items-center mt-2">
              ...
              <span className="text-[11px] tracking-widest uppercase text-violet-400">
...
const IntroScreen = ({ onStart }: { onStart: () => void }) => (
  <section className="relative py-24 md:py-32 overflow-hidden">
    <div className="absolute inset-0 bg-gradient-to-br from-violet-500/8 via-transparent to-violet-700/8 pointer-events-none" />
    <div className="absolute top-1/3 left-0 -translate-y-1/2 w-96 h-96 bg-violet-500/12 blur-[140px] rounded-full pointer-events-none" />
    <div className="absolute top-1/2 right-0 -translate-y-1/2 w-96 h-96 bg-violet-700/10 blur-[160px] rounded-full pointer-events-none" />

    <div className="container mx-auto px-6 relative z-10">
      <div className="max-w-3xl mx-auto text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-violet-500/10 border border-violet-500/20 mb-7">
          <Sparkles className="w-3.5 h-3.5 text-violet-400" />
          <span className="text-violet-300 font-medium tracking-widest uppercase text-[11px]">
            Auditoría confidencial · Academias &amp; Formación 2026
          </span>
        </div>
        ...
        <h1 className="text-4xl md:text-6xl font-bold mb-7 tracking-tight leading-[1.05]">
          ...
          de alumnos en tu academia.
        </h1>

        <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-5 leading-relaxed">
          Te hacemos las preguntas que importan sobre cómo gestionas leads, matriculaciones,
          comunicación y cobros. Recibes un{" "}
        ...
        <div className="flex flex-wrap gap-2.5 justify-center mb-10">
          {[
            ...
            return (
              ...
              >
                <Icon className="w-3.5 h-3.5 text-violet-400" /> {it.label}
              </div>
            );
          })}
        </div>
        ...
        <Button
          size="lg"
          onClick={onStart}
          className="bg-violet-600 hover:bg-violet-500 text-white h-16 px-10 text-lg gap-2 font-bold shadow-lg shadow-violet-900/40 transition-all hover:scale-[1.02]"
        >
        ...
        {[
          {
            ...
            desc: "Una nota de 0 a 100 con el nivel en el que está hoy tu academia frente a un centro de formación moderno bien sistematizado.",
          },
          {
            n: "02",
            title: "Diagnóstico por área",
            desc: "Score detallado en las 6 áreas clave: captación, matriculación, comunicación, retención, administración y gestión del profesorado.",
          },
          {
            n: "03",
          ...
          <div
            key={it.n}
            className="p-6 rounded-2xl border border-white/8 bg-white/[0.02] hover:bg-white/[0.04] hover:border-violet-500/20 transition-all"
          >
            <div className="text-violet-400 text-xs tracking-widest font-bold mb-3">{it.n}</div>
  ...
  <section className="py-12 md:py-20">
    <div className="container mx-auto px-6">
      <div className="max-w-2xl mx-auto">
        <div className="text-violet-400 text-[11px] tracking-widest font-bold uppercase mb-3">
          {q.cat}
        </div>
              ...
              return (
                <button
                  ...
                  className={`group text-left p-4 rounded-xl border transition-all flex items-center gap-4 ${
                    selected
                      ? "bg-violet-500/10 border-violet-500/50 shadow-[0_0_24px_rgba(139,92,246,0.18)]"
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] md:text-[11px] uppercase tracking-wider leading-tight text-center ${
                        selected ? "text-violet-300" : "text-gray-500 group-hover:text-gray-300"
                      }`}
                    >
                      {lbl}
                    </span>
        ...
        <div className="flex justify-between items-center gap-3 mt-8 pt-6 border-t border-white/5">
          ...
          <Button
            onClick={onNext}
            disabled={!canAdvance}
            className="bg-violet-600 hover:bg-violet-500 text-white disabled:bg-white/10 disabled:text-gray-500 gap-2 font-semibold"
          >
    ...
    <div className="container mx-auto px-6">
      <div className="max-w-2xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Ej. Directora de academia"
                className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 h-12"
              />
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            <div>
              ...
              <Input
                ...
                placeholder="Ej. Academia British School"
                className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 h-12"
              />
            </div>
            <div>
        ...
        <div className="flex justify-between items-center gap-3 mt-10 pt-6 border-t border-white/5">
          ...
          <Button
            onClick={onFinish}
            className="bg-violet-600 hover:bg-violet-500 text-white gap-2 font-semibold h-12 px-6"
          >
...
const ResultScreen = ({
  ...
  contact: Contact;
  result: { blockScores: Record<AcAuditBlockId, number>; global: number; level: AcAuditLevel };
  pdfBusy: boolean;
  onDownload: () => void;
}) => {
  const sorted = AC_AUDIT_BLOCK_KEYS.map((b) => ({ b, sc: result.blockScores[b] })).sort(
    (a, b) => b.sc - a.sc,
  );
  ...
  let ctaSub =
    "Una conversación de 30 minutos para ver qué frena tu captación y retención, qué ya tenéis montado y qué módulo puede generar impacto antes del próximo trimestre.";
  if (result.global < 50) {
    ctaTitle = "Cada alumno potencial sin sistema es una matrícula perdida.";
    ctaSub =
      "Tu nivel indica que hay módulos que pueden activarse en menos de dos semanas con impacto visible desde el primer mes. Agenda 30 min con nuestro equipo y preparamos un plan modular para tu academia.";
  } else if (result.global < 75) {
    ctaTitle = "Ya tienes una base. Falta hacerla sistema.";
    ctaSub =
      "Tienes piezas funcionando. Lo que falta es integrarlas y añadir la capa de retención y visibilidad que convierte leads en alumnos fieles sin que el equipo tenga que perseguir a nadie.";
  } else {
    ctaTitle = "Estás lista para la siguiente capa de optimización.";
    ctaSub =
      "Tu operativa ya está sólida. Hablemos sobre personalización por perfil de alumno, predicción de abandono con semanas de antelación y escalar el sistema a nuevas sedes.";
  }
    ...
    <section className="py-12 md:py-16">
      <div className="container mx-auto px-6">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Score hero */}
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#0d0618] to-[#1a0a2e] p-8 md:p-12 mb-6 grid grid-cols-1 md:grid-cols-[240px_1fr] gap-10 items-center">
            <div className="relative w-[200px] h-[200px] mx-auto">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-6xl font-bold text-violet-400 leading-none">
                  {result.global}
                </span>
            ...
            <div className="text-center md:text-left">
              <div className="text-violet-300 text-[11px] tracking-widest uppercase font-bold mb-2">
          ...
          <div className="rounded-3xl border border-white/8 bg-white/[0.02] p-7 md:p-8 mb-6">
            <div className="text-violet-400 text-[11px] tracking-widest font-bold uppercase mb-5">
              Score por área operativa
            </div>
            <div className="grid gap-3.5">
              {AC_AUDIT_BLOCK_KEYS.map((b) => {
                const sc = result.blockScores[b];
                const color =
                  sc < 40 ? "bg-red-500" : sc < 70 ? "bg-violet-700" : "bg-violet-400";
                const textColor =
                  sc < 40 ? "text-red-400" : sc < 70 ? "text-violet-500" : "text-violet-400";
                return (
                  <div
                    key={b}
                    className="grid grid-cols-[48px_1fr_72px] gap-4 items-center p-4 rounded-xl bg-white/[0.02] border border-white/5"
                  >
                    <div className="w-12 h-12 rounded-full bg-[#1a0a2e] border border-violet-500/30 flex items-center justify-center">
                      <span className="text-violet-400 font-bold text-sm">{b}</span>
                    </div>
                    <div>
                      <div className="text-white font-semibold text-sm">
                        {AC_AUDIT_BLOCKS[b].name}
                      </div>
                      <div className="text-xs text-gray-500 mt-0.5 mb-2">
                        {AC_AUDIT_BLOCKS[b].short}
                      </div>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <div className="p-5 rounded-2xl border border-red-500/25 bg-red-500/[0.04]">
                <h4 className="text-red-400 text-xs uppercase tracking-widest font-bold mb-3 flex items-center gap-1.5">
                  ...
                  Dónde se pierden alumnos y tiempo
                </h4>
                <ul className="space-y-2.5">
                  {weak.length ? (
                    weak.map((x) => (
          ...
          <div className="rounded-3xl border border-white/8 bg-white/[0.02] p-7 md:p-8 mb-6">
            <div className="text-violet-400 text-[11px] tracking-widest font-bold uppercase mb-1">
              Módulos prioritarios para tu academia
            </div>
            ...
            <div className="grid gap-3.5">
              {modules.map((m, i) => (
                <div
                  key={i}
                  className="p-5 rounded-2xl border-l-4 border-violet-500 bg-white/[0.02] border-t border-r border-b border-white/5"
                >
                  <div className="text-[10px] tracking-widest text-violet-400 font-bold uppercase mb-1">
                    {m.ref} · {AC_AUDIT_BLOCKS[m.block].name}
                  </div>
                  <h4 className="text-lg font-bold text-white mb-2">{m.name}</h4>
                  <p className="text-sm text-gray-400 leading-relaxed mb-3">{m.desc}</p>
                  <span className="inline-block px-3 py-1 rounded-full bg-violet-500/10 text-violet-300 text-[10px] uppercase tracking-widest font-bold border border-violet-500/20">
                    {m.impact}
                  </span>
                </div>
              ))}
          ...
          {/* CTA final */}
          <div className="rounded-3xl bg-gradient-to-br from-[#1a0a2e] to-[#0d0618] border border-violet-500/20 p-8 md:p-10 text-center text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="inline-flex items-center justify-center gap-2 px-7 h-12 rounded-full bg-violet-500 text-white font-bold hover:bg-violet-400 transition-all"
              >
                Agendar llamada de 30 min <ArrowRight className="w-4 h-4" />
              </a>
              <Link
                href="/sector/academias"
...
export default AuditoriaAcademias;
