...
import {
  ...
  CN_AUDIT_CANALES_LEADS,
  CN_AUDIT_PRIORIDADES,
  buildCnModulesByBlock,
  type CnAuditBlockId,
  type CnAuditLevel,
  type CnAuditQuestion,
...
const AuditoriaConstructoras = () => {
  ...
  const total = CN_AUDIT_QUESTIONS.length;
  const stepNum = cur + 1;
  const pct = Math.round((stepNum / (total + 1)) * 100);

  const q = CN_AUDIT_QUESTIONS[cur];
  ...
  const pick = (idx: number, val: string | number) => {
    const question = CN_AUDIT_QUESTIONS[idx];
  ...
  const finish = () => {
    ...
    const byBlock: Record<CnAuditBlockId, number[]> = {
      B1: [],
      B2: [],
      B3: [],
      B4: [],
    ...
    CN_AUDIT_QUESTIONS.forEach((qq, i) => {
      if (qq.type === "scale" && answers[i] !== undefined) {
        byBlock[qq.block].push(answers[i] as number);
      }
    });
    const blockScores = {} as Record<CnAuditBlockId, number>;
    CN_AUDIT_BLOCK_KEYS.forEach((b) => {
      const arr = byBlock[b];
      if (!arr.length) {
        blockScores[b] = 0;
        return;
    ...
    const global = Math.round(
      CN_AUDIT_BLOCK_KEYS.reduce((a, b) => a + blockScores[b], 0) / CN_AUDIT_BLOCK_KEYS.length,
    );
    const level =
      CN_AUDIT_LEVELS.find((l) => global >= l.min && global <= l.max) ?? CN_AUDIT_LEVELS[0];
    ...
    if (!GHL_WEBHOOK_URL.includes("LOCATION_ID")) {
      const payload = {
        ...
        custom_fields: {
          ...
          audit_tipo_empresa: CN_AUDIT_TIPO_EMPRESA[(answers[0] as string) || "A"] || "—",
          audit_num_unidades: CN_AUDIT_NUM_UNIDADES[(answers[1] as string) || "A"] || "—",
          audit_canales_leads: (Array.isArray(answers[2])
            ? (answers[2] as string[])
  ...
  const handleDownload = () => {
    ...
    downloadCnAuditPdf(state, setPdfBusy);
  };
    ...
    <div className="container mx-auto px-6 relative z-10">
      ...
      <div className="max-w-4xl mx-auto mt-24 grid grid-cols-1 md:grid-cols-3 gap-5">
        {[
          {
            n: "01",
            title: "Tu score de madurez comercial",
            desc: "Una nota de 0 a 100 con el nivel en el que está hoy tu comercialización frente a una promotora moderna bien sistematizada.",
          },
          {
            n: "02",
          ...
          },
          {
            ...
            desc: "Los 3-5 módulos que recomendamos activar primero — basados en las áreas más débiles de tu proceso y en tus prioridades del semestre.",
          },
        ].map((it) => (
          <div
            key={it.n}
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div>
              ...
              <Input
                ...
                placeholder="Ej. Promotora Mediterráneo"
                className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 h-12"
              />
            </div>
            <div>
              ...
              <Input
                ...
                placeholder="carlos@promotora.es"
                className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 h-12"
              />
            </div>
          </div>
...
const ResultScreen = ({
  ...
  contact: Contact;
  result: { blockScores: Record<CnAuditBlockId, number>; global: number; level: CnAuditLevel };
  pdfBusy: boolean;
  onDownload: () => void;
}) => {
  const sorted = CN_AUDIT_BLOCK_KEYS.map((b) => ({ b, sc: result.blockScores[b] })).sort(
    (a, b) => b.sc - a.sc,
  );
  ...
  let ctaTitle = "Convirtamos tu comercialización en sistema.";
  let ctaSub =
    "Una conversación de 30 minutos para ver qué frenos tiene tu proceso, qué ya tenéis montado y qué módulo puede generar impacto antes de que abra la siguiente fase de ventas.";
  if (result.global < 50) {
    ctaTitle = "Cada lead que llega sin sistema es una unidad en riesgo.";
    ctaSub =
      "Tu nivel indica que hay módulos que pueden activarse en menos de dos semanas con impacto visible desde el primer mes de comercialización. Agenda 30 min con nuestro equipo y preparamos un plan modular para tu promoción.";
  } else if (result.global < 75) {
    ctaTitle = "Ya tienes una base. Falta hacerla sistema.";
    ctaSub =
      "Tienes piezas funcionando. Lo que falta es integrarlas y añadir la capa de seguimiento y visibilidad que convierte leads fríos en reservas sin que el equipo tenga que perseguir a nadie.";
  } else {
    ctaTitle = "Estás listo para la siguiente capa.";
    ctaSub =
      "Tu comercialización ya está sólida. Hablemos sobre personalización avanzada por perfil de comprador, predicción de unidades en riesgo y replicar el sistema a nuevas promociones.";
  }
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <div className="p-5 rounded-2xl border border-red-500/25 bg-red-500/[0.04]">
                <h4 className="text-red-400 text-xs uppercase tracking-widest font-bold mb-3 flex items-center gap-1.5">
                  ...
                  Dónde se pierden leads y tiempo
                </h4>
                <ul className="space-y-2.5">
                  {weak.length ? (
                    weak.map((x) => (
                    ...
                    <li className="text-sm text-gray-400 leading-relaxed">
                      Tu comercialización está sólida en todos los frentes. La conversación ahora
                      es de optimización fina y escalado a nuevas promociones.
                    </li>
                  )}
                </ul>
              </div>
...
export default AuditoriaConstructoras;
